...
> Next: user commit-pause → ralph-loop the slices in a fresh session.
...
3. **The bake already carries `attrs` verbatim.** `export_publish_geojson.sh:77-95` emits
   ...
   collector reads at `main.js:1160` (`props.highlight === true`). **No bake change needed.**
...
- **Verify the left-tab curation fields render** for a starred reference row (blurb / status /
  ...
  revisit chip is live at `main.js:1396`, `renderPoiTab` renders blurb/kind/status
