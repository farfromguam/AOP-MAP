...
3. [~] Connect QGIS to `localhost:55432` and inspect source/feature tables — connection PROVEN from the
   host (`psql`, 2026-06-10) and the importable connection + runbook are ready
   (`../../spinup/qgis_connect_to_db.md`, `../../spinup/assets/qgis/aop_postgis_connection.xml`). The only
   step left is the user's `brew install --cask qgis` (a Bash agent is permission-blocked from installing
   apps). This is the tool the northstar always meant for editing — it lets the ~3k-line browser editor
   retire.
...
11. [ ] Continue the viewer-polish/code-health leftovers now carried by `../04_event_app/viewer_polish_followups.md`, after the Sprint 03 Pass 4 card moved to `../03_event_app/_done/code_health_pass_4.md`.
12. [X] Generate lidar-grade 5 ft contour lines for the 9-patch from the USGS 3DEP 1m DEM and wire them into the viewer. See `_done/lidar_contour_pipeline.md`.
13. [X] Add a web-viewer map editor (Terra Draw): draw/label/persist/export point POIs and polygon footprints (pavilions, buildings, etc.) in `website/index.html`. PostGIS write-back is the remaining follow-up. See `_done/poi_editor.md`.
14. [X] Add a searchable cemeteries layer. Confirmed the hole in the AOP boundary polygon is the Ellis Cemetery inholding (parcel `110 008.04`). See `_done/cemeteries_layer.md` and `brain/research/aop_ellis_cemetery.md`.
15. [X] Vectorize the satellite imagery into a forest land-cover layer (NAIP 2021 4-band -> texture classification -> forest polygons) and restyle the viewer into the Muted Earth palette. See `_done/landcover_layer.md`.
16. [X] Review/import 9-patch building footprints. FEMA USA Structures returned 202 footprints; OSM returned 11 `building=*` ways; TNMap FEMA BLE returned 0. Imported FEMA as a default-off viewer layer. See `_done/buildings_layer.md`.
17. [X] Add USDA NAIP as a tracing imagery layer and extend the viewer editor
    ...
    USDA's public 2023 Tennessee NAIP cache. See `_done/imagery_tracing_layer.md`.
18. [X] Add visitor support callout circles for South Pittsburg/Kimball and
    Monteagle services. See `_done/visitor_context_callouts.md`.
19. [X] Add time-weighted activity hotspots from first-party timestamped GPX.
    ...
    recorder/PostGIS steps are roadmapped in `_done/activity_hotspots.md`.
20. [X] Add a tag-driven proposed event schedule sidebar and default-off event
    ...
    See `_done/event_schedule_layer.md`.
