# Session Context Archive — 2026-05-27 → 2026-06-13

Archived changelog pruned out of `session_context.md` on 2026-06-13. This is the
raw session-by-session work log for the late-May → mid-June stretch (PWA QA
swarms, icon system, the Going-Gold ralph loop, viewer extraction into
`viewer_core.js`, banded-map draping, the `?tester=1` GPS offset, and the
left-rail drawer persistence through v70).

Read this only to retrace **why** something was built. The durable record for
each feature lives in its `tasks/*/_done/<feature>.md` build card,
`research/viewer.md`, `search_map.md`, or `spinup/mvp_runbook.md` — not here.
