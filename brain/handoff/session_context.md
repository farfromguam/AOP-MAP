...
Bucket A decisions (views/personas, always-on layer set, left-side hot button) still owed. User confirmed on 2026-05-23 to keep cutting through executable work before surfacing -- Bucket C now shipped on that direction. Remaining ready-to-pick: D (named-feature tagging, half curation/half implementation), E (buildings default-on, gated on A2), F (branding placement, gated on AOP permission ask). H (CSS/code Pass 3) folds into `../01_mvp/code_health_pass.md` once the editor churn settles.
