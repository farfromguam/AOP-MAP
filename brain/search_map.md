...
- trail names, trail number index, numbered trails, paper-map legend, Launch Pad / Major Tom / Ground Control, onX trail naming, Windrock vs AOP numbering: `research/aop_trail_name_index.md`
...
- feature tag binding, `#tag` on feature, `aop_feature_tags_v1`, tagToFeature, feature tag input, pavilion → 1010 building, schedule without coordinates, tag a building, tag a POI, named-feature tagging, seed `#pavilion`, refresh search index, refreshEventScheduleSearchIndex: `tasks/_done/02_edit/_done/named_feature_tagging.md`, then `research/viewer.md`
...
- brand logos, AOP logo, Rock Warblers logo, asset permission, transparent logo, icon-size slider, branding assets: `tasks/04_event_app/brand_assets_and_permissions.md`, then `tasks/_done/02_edit/_done/branding.md`
...
- data maturity, gold data, silver, bronze, medallion tier, tier, locked data, unlock to edit, `_meta.maturity`, per-feature maturity, `stamp_maturity.py`, `set_feature_maturity.py`, rename data sources, ellis gold, buildings gold, shower house bronze, production data-tier alert, `auditProductionTiers`, alert bronze silver in production, gold/silver editor sections, maturity badge, which data is gold, do-not-gold: `research/data_maturity_tiers.md`, then `mvp/scripts/stamp_maturity.py` / `mvp/scripts/set_feature_maturity.py`, then `tasks/_done/02_edit/_done/six_item_viewer_batch_20260614.md`
- trail display name, number-first label, "1 Launchpad", trail label and search match, `trailDisplayName`, `display_name`, search shows number, borderless tree cover, remove landcover border, search/event active item, selection persists, de-throne, highlight holds, proline at firepit, pro-line before the fire, `#firepit`, firepit tag: `tasks/_done/02_edit/_done/six_item_viewer_batch_20260614.md`, then `research/viewer.md`
- rename data source files, bronze_/silver_/gold_/delete_ filename prefix, `gold_aop_trail_network.geojson`, tier-prefixed geojson, `rename_data_medallion.py`, re-run after bake, data editor page tier groups, data_editor.html optgroups, group file picker by tier, `data_editor_map.js`: `tasks/_done/02_edit/_done/six_item_viewer_batch_20260614.md` (rename addendum), then `research/data_maturity_tiers.md`
- common feature schema, CMFS, `aop-cmfs-v1`, canonical fields, crosswalk, `rebake_canonical.py`, re-bake, `bake_panel_overrides.py`, panel overrides, one data contract: `research/common_feature_schema.md`
...
- satellite Illustrator trace, hand trace the satellite, 9-patch export for Illustrator, trace in Illustrator, gold trails export, waypoints + buildings export, layered SVG over satellite, names are the layer names, re-import by layer name, `export_illustrator_trace.py`, `import_illustrator_trace.py`, UTM 16N frame, NAIP backdrop, ai_escape: `tasks/14_illustrator_trace/satellite_illustrator_export.md`, then `tasks/20_deferred/paper_map_trail_extraction.md`
- paper map trail extraction, vectorize the SFWDA raster, line trace trails, extract trails from paper map, difficulty markers, green circle blue square black triangle, marker detection, skeletonize, georeference the warp, `PaperWarp`, trail numbers, trail_number, attach trail numbers, number the markers, contiguous trail, contiguous trail under the markers, stitch trails, numbered trails layer, `sfwda_numbered_trails.geojson`, full pipeline runner, `extract_paper_trails.py`, `vectorize_paper_trails.py`, `paper_trace_warp.py`, `attach_trail_numbers.py`, `build_numbered_trails.py`, `run_paper_trace_pipeline.sh`, editable SVG export, edit trails in Inkscape, rectified raster, warp baked in, layered SVG, round-trip import, trail-number labels in viewer, `export_trace_svg.py`, `import_trace_svg.py`, `sfwda_trails_edited.geojson`: `tasks/04_event_app/paper_map_trail_extraction.md`
...
- export landcover to SVG, edit vegetation in Illustrator, hand-resolve tree polygons, single green for treecover, editable canopy SVG over satellite, dissolved canopy not grid pieces, Vegetation layer, re-import landcover, `export_landcover_svg.py`, `import_landcover_svg.py`, `verify_landcover_svg_roundtrip.py`, `RasterFrame` (shared UTM-16N frame + satellite backdrop), `vegetation_features` (shared viewer fill/outline contract), `brain/output/landcover_trace/`: `tasks/01_mvp/_done/landcover_layer.md` (vegetation → editable SVG round-trip)
